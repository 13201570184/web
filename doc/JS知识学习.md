...
### 2. node.insertBefore(child,指定元素)   node.insertBefore(添加的元素,node.children[0]) (将添加的元素是node里面的第一个子元素)
...
### 原理就是：
### (1). 当我们手指触摸屏幕，记录当前触摸时间https://pan.baidu.com/s/1KotiABx0CCj-X5kAdxG9yg 
...
### JS设置表单验证里匹配电话号码的方式  /^1[3-8]\d{9}$/
### JS设置表单验证里匹配QQ号码的方式  /^[1-9]\d{4,9}$/
### JS设置表单验证里匹配昵称的方式(纯文字类的昵称)   /^[\u4e00-\u9fa5]{2,8}$/
### JS设置表单验证里匹配手机短信验证码的方式(6位号数字)   /^\d{6}$/
### JS设置表单验证里匹配设置登陆密码的方式   /^[a-zA-Z0-9_-]{6,16}$/
